...
fn synth_corpus_decodes_at_comfortable_snr() {
    ...
    let entries = load_synth_corpus(&workspace, &manifest_path).expect("manifest must load");
    ...
    for entry in &entries {
        ...
        if decodes
            .iter()
            .any(|d| d.message.contains(&entry.encoded_message))
        {
    ...
    assert!(
        comfortable_total > 0,
        "should have some comfortable-SNR entries"
    );
    let rate = comfortable_recovered as f64 / comfortable_total as f64;
    println!("comfortable-SNR recovery: {comfortable_recovered}/{comfortable_total} = {rate:.2}");
    assert!(
        rate >= 0.80,
