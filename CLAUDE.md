...
- **Phase 5** (in progress): Full autonomous QSO loop — complete a CQ→grid→report→RR73 exchange end-to-end autonomously.
  ...
  - **Remaining**: (a) smart-TX-offset for autonomous QSOs (needs an RX-match-freq vs TX-freq split in `QsoState`; v1 answers Tx=Rx); (b) a full live-coordinator integration test; (c) on-air A/B validation — operator-gated (needs antenna). See `docs/qso-scenario-catalog-2026-06-16.md` (Phase 5 status).
