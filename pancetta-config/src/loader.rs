...
impl ConfigLoader {
    ...
    pub fn load(&self) -> ConfigResult<Config> {
        ...
        for source in sorted_sources {
            match self.load_source(&source) {
                ...
                Err(e) => {
                    if source.required {
                        error!("Failed to load required configuration source '{}': {}", source.name, e);
                        return Err(e);
                    } else {
                        warn!("Failed to load optional configuration source '{}': {}", source.name, e);
                    }
                }
            }
        }
        ...
        info!("Configuration loaded successfully from {} sources", sources.len());
        Ok(config)
    }
    ...
    fn discover_sources(&self) -> ConfigResult<Vec<ConfigSource>> {
        ...
        for (index, search_path) in self.search_paths.iter().enumerate() {
            let config_files = ["pancetta.toml", "config.toml", "pancetta.json", "config.json"];
    ...
    fn parse_json(&self, content: &str) -> ConfigResult<Config> {
        ...
        serde_json::from_str(&expanded_content)
...
callsign = "K1CACHE"
    ...
    fn test_is_config_file() {
        ...
        assert!(ConfigLoader::is_config_file(&PathBuf::from("settings.json")));
