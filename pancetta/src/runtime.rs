...
impl PancettaRuntime {
    ...
    pub fn new(config: RuntimeConfig) -> Result<Self> {
        info!("Creating Pancetta runtime with {} worker threads", config.worker_threads);
        ...
        F::Output: Send + 'static,
    {
        ...
        self.runtime.spawn(async move {
            ...
            if duration > Duration::from_millis(1) {
                warn!("Critical task '{}' took {:.2}ms (expected <1ms)", task_name, duration.as_secs_f64() * 1000.0);
            }
    ...
    pub async fn start_metrics_collection(&self) -> Result<()> {
        ...
        self.spawn("metrics_collector", async move {
            ...
            while !is_shutdown.load(Ordering::Relaxed) {
                ...
                debug!("Runtime metrics updated - Active tasks: {}, Memory: {} MB", 
                       current_active, memory_usage / 1024 / 1024);
            }
        });
    ...
    pub async fn shutdown(&self, timeout: Duration) -> Result<()> {
        ...
        if remaining_tasks > 0 {
            warn!("Shutting down with {} active tasks remaining", remaining_tasks);
        }
        
        // Runtime will be dropped, triggering shutdown
        info!("Runtime shutdown completed in {:.2}s", start.elapsed().as_secs_f64());
        Ok(())
    }
    ...
    pub async fn is_healthy(&self) -> bool {
        ...
        if metrics.avg_task_duration_us > 10_000.0 { // 10ms average is concerning
            warn!("Runtime health check failed: average task duration too high ({:.2}ms)", 
                  metrics.avg_task_duration_us / 1000.0);
            return false;
        }
        
        // Check memory usage (basic check)
        if metrics.memory_usage_bytes > 1024 * 1024 * 1024 { // 1GB limit
            warn!("Runtime health check failed: memory usage too high ({} MB)", 
...
fn set_thread_priority(priority: i32) {
    ...
    unsafe {
        ...
        if result != 0 {
            warn!("Failed to set thread priority: {}", std::io::Error::last_os_error());
        } else {
