...
### Decode effort control (budget-governed anytime decoder)

A single decode window runs as an *anytime* algorithm: candidate stations are decoded in a fixed priority order (top sync-score candidates first, then successively deeper recovery stages — cross-cycle, multi-pass, joint-pair, AP), and the whole pass can be capped to a per-window wall-clock budget without changing the order results arrive in. Given the same audio and an unlimited budget, the decoder always produces the same output as before this work — the budget only controls how much of that fixed-order work list gets a chance to run. The operator selects an effort preset — `Eco` (fastest, floor-decode only), `Standard`, `Deep` (more decode passes, better recall), `Max` (unlimited, always full recall), or `Auto` (the default — derives a budget from the auto-probed hardware tier so the same install self-tunes across a fast desktop and a slow headless MiniPC) — via the `[decoder]` config section or live with the `e` key in the TUI, which shows the active preset in a status chip. This replaces the older fixed-per-hardware-tier decode presets with an operator-adjustable knob.
