...
pub fn render_station_info(f: &mut Frame<'_>, area: Rect, app: &App) -> Result<()> {
    ...
    // full-width-waterfall layout makes this panel short and the bottom gets
    // truncated first. The operator ALWAYS needs the band and will RARELY need
    // their own callsign or equipment, so:
    //   1. Operating parameters (Freq / Band / Mode) — always visible
    //   2. Audio monitoring + S-meter/rig — operational
    //   3. Autonomous status — operational
    //   4. Station details (call / grid / power / rig) — rarely needed, demoted
    //   5. Equipment / position / stats — rarely needed, truncated first
    let chunks = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(2), // Operating parameters (Freq / Band / Mode)
            Constraint::Length(2), // Audio monitoring
            Constraint::Length(2), // Autonomous status
            Constraint::Length(3), // Station details (call / grid / power / rig)
    ...
    // Operating Parameters (Freq / Band / Mode) — first, always visible.
    render_operating_parameters(f, chunks[0], app);

    // Audio Monitoring Status
    render_audio_status(f, chunks[1], app);

    // Autonomous Operator Status
    render_autonomous_status(f, chunks[2], app);

    // Station Details (call / grid / power / rig) — rarely needed, demoted.
    render_station_details(f, chunks[3], app);

    // Equipment Information — rarely needed, truncated first.
