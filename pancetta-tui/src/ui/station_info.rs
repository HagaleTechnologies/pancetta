...
fn render_station_details(f: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    let lines = vec![
        Line::from(vec![
            Span::styled("Call Sign: ", Style::default().fg(app.theme.foreground_color())),
            Span::styled(
                &station.call_sign,
                Style::default().fg(app.theme.success_color()).add_modifier(Modifier::BOLD)
            ),
            Span::raw("    "),
            Span::styled("Grid Square: ", Style::default().fg(app.theme.foreground_color())),
            Span::styled(
                &station.grid_square,
        ...
        Line::from(vec![
            ...
            Span::raw("    "),
            Span::styled("Antenna: ", Style::default().fg(app.theme.foreground_color())),
            Span::styled(
                &station.antenna,
...
fn render_operating_parameters(f: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    let band_info = app.config.get_current_band(station.operating_frequency)
        .map(|b| b.name.as_str())
        .unwrap_or("Unknown");

    let lines = vec![
        Line::from(vec![
            Span::styled("Frequency: ", Style::default().fg(app.theme.foreground_color())),
            Span::styled(
                app.config.ui.frequency_format.format_frequency(station.operating_frequency),
                Style::default().fg(app.theme.warning_color()).add_modifier(Modifier::BOLD)
            ),
            Span::raw("  "),
        ...
        Line::from(vec![
            ...
            Span::styled(
                if app.is_monitoring { "MONITORING" } else { "STANDBY" },
                Style::default().fg(
...
fn render_audio_status(f: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    let device_lines = vec![
        Line::from(vec![
            Span::styled("Device: ", Style::default().fg(app.theme.foreground_color())),
        ...
        Line::from(vec![
            ...
            Span::styled(format!("{} Hz", sample_rate), Style::default().fg(app.theme.foreground_color())),
        ]),
    ];
...
fn render_equipment_info(f: &mut Frame<'_>, area: Rect, app: &App) {
    ...
    let lines = vec![
        Line::from(vec![
            Span::styled("Equipment: ", Style::default().fg(app.theme.foreground_color())),
            Span::styled(&station.rig, Style::default().fg(app.theme.foreground_color())),
            Span::raw(" + "),
            Span::styled(&station.antenna, Style::default().fg(app.theme.foreground_color())),
        ]),
    ];
    ...
    if area.height > 1 && app.config.ui.show_coordinates {
        all_lines.push(Line::from(vec![
            Span::styled("Position: ", Style::default().fg(app.theme.foreground_color())),
            Span::styled(
    ...
    if area.height > 2 {
        ...
        all_lines.push(Line::from(vec![
            ...
            Span::styled(format!("{} msgs", total_messages), Style::default().fg(app.theme.accent_color())),
            Span::raw(", "),
            Span::styled(format!("{} DX", dx_stations), Style::default().fg(app.theme.accent_color())),
        ]));
    }
