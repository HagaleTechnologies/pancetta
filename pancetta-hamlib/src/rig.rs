...
impl Rig {
        ...
        T: Send,
    {
        ...
        debug!("Starting rig operation {} with timeout {}ms", op_id, config.timeout_ms);
        ...
        loop {
            ...
            if !self.connected.load(Ordering::Relaxed) {
                if config.auto_reconnect {
                    warn!("Rig not connected, attempting reconnection for operation {}", op_id);
                    if let Err(e) = self.connect().await {
                        error!("Failed to reconnect for operation {}: {}", op_id, e);
                        return RigOperationResult {
                            result: Err(anyhow!("Rig not connected and reconnection failed: {}", e)),
                            retries,
            ...
            match result {
                Ok(value) => {
                    debug!("Operation {} completed successfully after {} retries", op_id, retries);
                    return RigOperationResult {
                        result: Ok(value),
                        retries,
                        duration: start_time.elapsed(),
                ...
                Err(e) => {
                    warn!("Operation {} failed (attempt {}): {}", op_id, retries + 1, e);
                    ...
                    if retries >= config.retry_count {
                        ...
                        self.update_connection_state(ConnectionState::Error, Some(e.to_string())).await;
    ...
    fn init_hamlib_handle(&self) -> Result<RigHandle> {
        ...
        unsafe {
            ...
            if !is_hamlib_success(result) {
                ...
                return Err(anyhow!("Failed to set device path: {}", hamlib_error_message(result)));
            }
            
            // Set baud rate if specified
            if let Some(baud_rate) = config.baud_rate.or(Some(config.model.capabilities().default_baud_rate)) {
                let baud_str = baud_rate.to_string();
                let baud_cstr = string_to_c_str(&baud_str)
                    .map_err(|e| anyhow!("Invalid baud rate: {}", e))?;
                let result = rig_set_conf(handle.as_ptr(), 2, baud_cstr.as_ptr()); // TOKEN_SERIAL_SPEED = 2
                if !is_hamlib_success(result) {
                    warn!("Failed to set baud rate {}: {}", baud_rate, hamlib_error_message(result));
                }
            }
...
impl RigControl for Rig {
    ...
    async fn connect(&self) -> Result<()> {
        ...
        self.update_connection_state(ConnectionState::Connecting, None).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                let result = rig_open(handle.as_ptr());
                if is_hamlib_success(result) {
                    Ok(())
                } else {
                    Err(anyhow!("Failed to open rig: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        match result.result {
            Ok(()) => {
                *self.handle.lock() = Some(handle);
                ...
                self.update_connection_state(ConnectionState::Connected, None).await;
    ...
    async fn disconnect(&self) -> Result<()> {
            ...
            let result = self.execute_operation(move || {
                unsafe {
                    ...
                    if !is_hamlib_success(cleanup_result) {
                        warn!("Error cleaning up rig: {}", hamlib_error_message(cleanup_result));
                    }
        ...
        self.connected.store(false, Ordering::Relaxed);
        self.update_connection_state(ConnectionState::Disconnected, None).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to set frequency: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to get frequency: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to set mode: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to get mode: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to set VFO: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to get VFO: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to set PTT: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to get PTT: {}", hamlib_error_message(result)))
                }
            }
        }).await;
    ...
    async fn set_power_level(&self, level: f32) -> Result<()> {
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                let result = rig_set_level(handle.as_ptr(), RIG_VFO_CURR, RIG_LEVEL_RFPOWER, 
                                         std::ptr::addr_of!(power_val) as *const c_void);
                if is_hamlib_success(result) {
                    Ok(())
                } else {
                    Err(anyhow!("Failed to set power level: {}", hamlib_error_message(result)))
                }
            }
        }).await;
    ...
    async fn get_power_level(&self) -> Result<f32> {
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                let result = rig_get_level(handle.as_ptr(), RIG_VFO_CURR, RIG_LEVEL_RFPOWER, 
                                         std::ptr::addr_of_mut!(power_val) as *mut _ as _);
                if is_hamlib_success(result) {
                    Ok(power_val)
                } else {
                    Err(anyhow!("Failed to get power level: {}", hamlib_error_message(result)))
                }
            }
        }).await;
    ...
    async fn get_s_meter(&self) -> Result<i32> {
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                let result = rig_get_level(handle.as_ptr(), RIG_VFO_CURR, RIG_LEVEL_STRENGTH, 
                                         std::ptr::addr_of_mut!(s_val) as *mut _ as _);
                if is_hamlib_success(result) {
                    Ok(s_val)
                } else {
                    Err(anyhow!("Failed to get S-meter: {}", hamlib_error_message(result)))
                }
            }
        }).await;
    ...
    async fn get_swr(&self) -> Result<f32> {
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                let result = rig_get_level(handle.as_ptr(), RIG_VFO_CURR, RIG_LEVEL_SWR, 
                                         std::ptr::addr_of_mut!(swr_val) as *mut _ as _);
                if is_hamlib_success(result) {
                    Ok(swr_val)
                } else {
                    Err(anyhow!("Failed to get SWR: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to set memory channel: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to get memory channel: {}", hamlib_error_message(result)))
                }
            }
        }).await;
        ...
        let result = self.execute_operation(move || {
            unsafe {
                ...
                if is_hamlib_success(result) {
                    ...
                    Err(anyhow!("Failed to set scan: {}", hamlib_error_message(result)))
                }
            }
        }).await;
