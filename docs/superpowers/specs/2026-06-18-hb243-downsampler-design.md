...
### Phase 1 — baseband mixer + decimator, unit-tested in isolation (no decode wiring)  ✅ DONE 2026-06-22

Implemented in `pancetta-ft8/src/baseband.rs` (standalone module, not wired into
the decode path): `design_decimation_lowpass` (Kaiser windowed-sinc, β from 60 dB
atten, ~819 taps, unity DC, linear-phase), `baseband_extract`/`baseband_extract_with`
(complex down-mix → FIR → decimate by 60 → 200 Hz). 5 unit tests pass: FIR
well-formedness (odd/unity-DC/symmetric), filter response (<0.5 dB ripple 0–50 Hz,
≥50 dB stopband at ≥100 Hz), single-tone-lands-at-DC, off-carrier-tone-at-+25 Hz,
and the transmit-gated full-FT8 per-symbol parity recovery (≥90% symbols correct).
Reference uses direct FIR conv + decimate (cost ≈ N_taps·N_bb ≈ 819·2528 ≈ 2.1M
mul-add/candidate); the cached-FFT production form (§2.4) is the Phase-2+
optimization. Clean-room (public DSP only). **Gate to Phase 2: PASSED.**
