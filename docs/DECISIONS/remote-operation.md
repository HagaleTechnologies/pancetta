...
## Remote-TX arm-gate drop gains local diagnostic visibility (dispensa Q-0051 Phase A), 2026-07-30
...
**Phases B (AuditLog-append the drop, matching the existing `Arm`-rejection `TxDenied` pattern) and
C (relay a client-visible signal — reuses the already-frozen `ServerEvent::Error`, no contract change
needed, via the established `relay_to_gateway` additive dual-destination pattern) are scoped but not
yet built.** Full technical trail: dispensa
