...
## Unattended operation & process supervision

For an always-on station you must run pancetta **headless under an OS
supervisor that restarts it automatically.** This is non-optional for
unattended operation: a panic anywhere in the pipeline — or a native abort
from the vendored `ft8_lib` C decoder, which no in-process Rust handler can
catch — would otherwise take the station off-air until a human relaunches it.

Pancetta defends against in-process Rust panics in two ways (each decode window
is wrapped in `catch_unwind`, so one bad window is logged and skipped rather
than fatal; the release build uses `panic = "unwind"` so this works), but the
OS supervisor is the backstop that recovers from everything else.

Ship-provided units (edit the binary path inside each):

- **Linux / MiniPC:** `packaging/systemd/pancetta.service` — `Restart=always`,
  `RestartSec=2`, crash-loop rate limit. Install as a `--user` service and
  `loginctl enable-linger`. Watch with `journalctl --user -u pancetta -f`.
- **macOS:** `packaging/launchd/net.hagale.pancetta.plist` — `KeepAlive` with a
  `ThrottleInterval`. Install in `~/Library/LaunchAgents`.
- **Windows:** wrap `pancetta.exe --headless` with
  [NSSM](https://nssm.cc/) (`nssm install Pancetta …`, set *Exit actions →
  Restart*) or a Task Scheduler task with trigger *At startup* and
  *Restart on failure*. The per-user profile keeps `~/.pancetta` private.

Always run the supervised station with `--headless` (no TUI). Use the TUI
interactively for setup, manual operation, and monitoring.
