...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    for (i, (their_call, their_grid, frequency, our_report, their_report)) in contest_qsos.iter().enumerate() {
        ...
        let qso_id = system.respond_to_cq(their_call.to_string(), *frequency).await?;
        ...
        system.process_message(
            their_exchange,
            format!("W1ABC {} {:03} {:03}", their_call, their_report + 35, serial * 10),
            *frequency,
    ...
    if let Some(ref logger) = system.logger {
        ...
        let band_qsos = logger.search_qsos(&band_filter, &QueryOptions::default()).await?;
        ...
        let strong_qsos = logger.search_qsos(&strong_filter, &QueryOptions::default()).await?;
        ...
        let grid_qsos = logger.search_qsos(&grid_filter, &QueryOptions::default()).await?;
