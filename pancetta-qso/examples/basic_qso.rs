...
async fn main() -> Result<(), Box<dyn Error>> {
    ...
    println!(
        "✓ Started CQ call on {:.3} MHz (QSO ID: {})",
    ...
    println!(
        "  Would send: {}",
        utils::generate_ft8_message(&report_message, "W1ABC")?
    );
    ...
    println!(
        "  Would send: {}",
        utils::generate_ft8_message(&confirmation_message, "W1ABC")?
    );
    ...
    println!(
        "Their call: {}",
        metadata.their_callsign.as_deref().unwrap_or("UNKNOWN")
    );
    println!("Frequency: {:.3} MHz", metadata.frequency / 1_000_000.0);
    println!("Mode: {}", metadata.mode);
    println!(
        "Our grid: {}",
        metadata.grids.ours.as_deref().unwrap_or("UNKNOWN")
    );
    println!(
        "Their grid: {}",
        metadata.grids.theirs.as_deref().unwrap_or("UNKNOWN")
    );
    println!(
        "Reports sent/received: {:+}/{:+} dB",
    ...
    let test_freqs = [
        1840000.0, 3573000.0, 7074000.0, 14074000.0, 21074000.0, 28074000.0, 50313000.0,
    ];
    ...
    for (signal, noise) in &test_signals {
        ...
        println!(
            "Signal {:.1} dB, Noise {:.1} dB = {:+} dB report",
            signal, noise, report
        );
    }
