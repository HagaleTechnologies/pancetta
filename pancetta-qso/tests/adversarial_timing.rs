...
async fn sim_with_max_calls(max_calls: u32) -> Sim {
    Sim::with_config(QsoManagerConfig {
        ...
        timeouts: TimeoutConfig {
            ...
            // These multi-minute timing scenarios deliberately disable the
            // keep-call watchdog; disable the (tighter) repetitive-TX cap too so
            // it doesn't retire QSOs mid-scenario. The cap has its own tests.
...
async fn c12_watchdog_is_per_qso_not_per_step() {
    // The call-count cap is PER-QSO across re-calls in the INITIAL-CALL phase
    // (RespondingToCq): re-sending our grid each slot accumulates toward the
    // SAME cap (4) — a per-slot reset would let us pound a non-answering DX
    // forever. The DX here never answers, so we stay in RespondingToCq and the
    // cap binds.
    //
    // (Note: once the DX ENGAGES — sends a report → SendingReport — the cap no
    // longer applies, so the DX's eventual RR73 isn't dropped; that engaged-
    // exchange behavior is the 9K2MP on-air fix, covered by qso_manager's
    // engaged_sending_report_* tests.)
    ...
    // DX never answers — we keep re-calling our grid in RespondingToCq.
    sim.tick_n(8).await;

    let tl = sim.into_timeline();
    tl.assert_failed_with(QsoFailureReason::Timeout);
    tl.assert_not_completed_with(dx);
    // Grid re-calls are bounded by the per-QSO cap (4), NOT reset to a fresh
    // per-slot budget (a per-slot reset would run well past this).
    let grids = tl.count_transmitted_containing("VB7F K5ARH EM10");
    assert!(
        grids <= 5,
        "per-QSO cap must bound grid re-calls in RespondingToCq (≤5), got \
         {grids} — a per-slot reset would exceed this\n{tl}"
    );
}
