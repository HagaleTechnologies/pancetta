...
async fn scenario_11_fade_out_retired_by_watchdog() {
    // Short TIME watchdog so the test runs in a few fast slots. Once the DX has
    // ENGAGED (its report → we're in SendingReport), the call-count cap no
    // longer applies (it would otherwise drop the QSO before a slow DX's RR73 —
    // the 9K2MP on-air fix); a true fade-out is now bounded by the time watchdog.
    let mut sim = Sim::with_config(QsoManagerConfig {
        ...
        timeouts: TimeoutConfig {
            ...
            manual_call_watchdog_minutes: 2, // time watchdog bounds the faded exchange
            ..Default::default()
        },
        ..Default::default()
    })
