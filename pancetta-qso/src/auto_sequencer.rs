...
use crate::states::*;
use crate::exchange::{MessageExchange, ExchangeError};
...
impl AutoSequencer {
    ...
    pub async fn process_event(&self, event: QsoEvent) -> Result<(), AutoSequencerError> {
        match event {
            QsoEvent::StateChanged { qso_id, old_state, new_state, .. } => {
                self.handle_state_change(qso_id, old_state, new_state).await?;
            }
    ...
    pub async fn evaluate_cq_call(
        ...
        if !self.should_respond_to_station(callsign, signal_strength, grid).await {
        ...
        if snr < self.config.signal_thresholds.min_cq_response_snr {
            debug!("Not responding to {} - signal too weak ({}dB)", callsign, snr);
            return Ok(SequenceDecision::NoAction);
        }
        
        info!("Auto-responding to CQ from {} on {:.1} Hz", callsign, frequency);
    ...
    pub async fn execute_action(&self, action: SequenceAction) -> Result<(), AutoSequencerError> {
        match action {
            SequenceAction::StartCq { frequency } => {
                let qso_id = self.qso_manager.start_cq(frequency).await
            ...
            SequenceAction::RespondToCq { callsign, frequency } => {
                let qso_id = self.qso_manager.respond_to_cq(callsign, frequency).await
                    .map_err(|e| AutoSequencerError::QsoManager { source: e })?;
    ...
    async fn handle_state_change(
        ...
        debug!("QSO {} state changed: {:?} -> {:?}", qso_id, old_state, new_state);
    ...
    async fn handle_message_received(
        ...
        debug!("Message received for QSO {}: {:?}", qso_id, message.message_type);
    ...
    async fn should_respond_to_station(
        ...
        if !filters.allowed_grid_squares.is_empty() {
            if let Some(grid) = grid {
                if !filters.allowed_grid_squares.iter().any(|allowed| grid.starts_with(allowed)) {
                    return false;
                }
            } else {
    ...
    async fn start_sequence(&self, qso_id: QsoId) {
        ...
        self.active_sequences.write().await.insert(qso_id, sequence_state);
    ...
    async fn check_sequence_timeouts(&self) -> Result<(), AutoSequencerError> {
        let now = Utc::now();
        let timeout_duration = chrono::Duration::seconds(self.config.timing.response_timeout as i64);
...
mod tests {
    use super::*;
    use crate::qso_manager::{QsoManagerConfig, TimeoutConfig, AutoSequenceConfig as QsoAutoConfig, DuplicateCheckConfig};
    ...
    async fn test_cq_evaluation() {
        ...
        let decision = sequencer.evaluate_cq_call(
            "K1DEF",
            14074000.0,
            Some("FN31"),
            -10.0 // Good signal
        ...
        match decision {
            SequenceDecision::Action(SequenceAction::RespondToCq { callsign, frequency }) => {
