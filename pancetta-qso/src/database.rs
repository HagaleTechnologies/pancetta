...
use rusqlite::{params, Connection, Row, Transaction, OptionalExtension};
...
impl QsoDatabase {
    ...
    pub fn insert_qso(&mut self, progress: &QsoProgress) -> Result<i64, DatabaseError> {
        ...
        match result {
            Ok(id) => {
                tx.commit().map_err(|e| DatabaseError::Sqlite { source: e })?;
                Ok(id)
            }
            Err(e) => {
                tx.rollback().map_err(|e| DatabaseError::Sqlite { source: e })?;
                Err(e)
            }
        }
    }
    ...
    pub fn get_qso(&self, qso_id: QsoId) -> Result<QsoDatabaseRecord, DatabaseError> {
        ...
        let record = stmt.query_row(params![qso_id.to_string()], |row| {
    ...
    pub fn search_qsos(
        ...
        let rows = stmt.query_map(rusqlite::params_from_iter(params.iter()), |row| self.row_to_record(row))
            .map_err(|e| DatabaseError::Sqlite { source: e })?;
    ...
    pub fn get_statistics(&self) -> Result<DatabaseStats, DatabaseError> {
        let total_qsos: u64 = self.connection.query_row(
            "SELECT COUNT(*) FROM qsos",
            [],
            |row| row.get::<_, i64>(0)
        ).map_err(|e| DatabaseError::Sqlite { source: e })? as u64;
        
        let confirmed_qsos: u64 = self.connection.query_row(
            "SELECT COUNT(*) FROM qsos WHERE json_extract(metadata, '$.end_time') IS NOT NULL",
            [],
            |row| row.get::<_, i64>(0)
        ).map_err(|e| DatabaseError::Sqlite { source: e })? as u64;
        
        let unique_callsigns: u64 = self.connection.query_row(
        ...
        let countries_worked: u64 = self.connection.query_row(
        ...
        let grid_squares_worked: u64 = self.connection.query_row(
        ...
        let first_qso = self.connection.query_row(
            "SELECT MIN(json_extract(metadata, '$.start_time')) FROM qsos",
            [],
            |row| {
                let date_str: Option<String> = row.get(0)?;
                Ok(date_str.and_then(|s| DateTime::parse_from_rfc3339(&s).ok().map(|dt| dt.with_timezone(&Utc))))
            }
        ).map_err(|e| DatabaseError::Sqlite { source: e })?;
        
        let last_qso = self.connection.query_row(
        ...
        // Get database size
        let database_size = self.connection.query_row(
    ...
    pub fn check_duplicate(
        ...
        let duplicate_id: Option<String> = self.connection.query_row(
    ...
    fn initialize_schema(&mut self) -> Result<(), DatabaseError> {
        ...
        self.schema_version = self.connection.query_row(
            "SELECT COALESCE((SELECT value FROM metadata WHERE key = 'schema_version'), '0')",
            [],
            |row| row.get::<_, String>(0)?.parse::<u32>().map_err(|_| rusqlite::Error::InvalidColumnType(0, "key".to_string(), rusqlite::types::Type::Text))
        ).unwrap_or(0);
    ...
    fn migrate_schema(&mut self, target_version: u32) -> Result<(), DatabaseError> {
        info!("Migrating database schema from version {} to {}", 
    ...
    fn insert_qso_tx(tx: &Transaction, progress: &QsoProgress, adif_processor: &AdifProcessor) -> Result<i64, DatabaseError> {
        let qso_id = progress.metadata.qso_id;
        ...
        let adif_qso = adif_processor.qso_to_adif(&progress.metadata, 
                                                      progress.metadata.contest_info.as_ref());
    ...
    fn update_qso_tx(tx: &Transaction, progress: &QsoProgress, adif_processor: &AdifProcessor) -> Result<(), DatabaseError> {
        let qso_id = progress.metadata.qso_id;
    ...
    fn row_to_record(&self, row: &Row) -> rusqlite::Result<QsoDatabaseRecord> {
        ...
        let metadata: QsoMetadata = serde_json::from_str(&metadata_json)
            .map_err(|e| rusqlite::Error::FromSqlConversionFailure(
                0, rusqlite::types::Type::Text, Box::new(e)
            ))?;
        
        let final_state: QsoState = serde_json::from_str(&state_json)
            .map_err(|e| rusqlite::Error::FromSqlConversionFailure(
                0, rusqlite::types::Type::Text, Box::new(e)
            ))?;
        
        let adif_data: AdifQso = serde_json::from_str(&adif_json)
            .map_err(|e| rusqlite::Error::FromSqlConversionFailure(
                0, rusqlite::types::Type::Text, Box::new(e)
            ))?;
    ...
    fn build_where_clause(&self, filter: &QsoFilter) -> Result<(String, Vec<rusqlite::types::Value>), DatabaseError> {
        ...
        if let Some(min_signal) = filter.min_signal_strength {
            conditions.push("CAST(json_extract(metadata, '$.reports.received') AS INTEGER) >= ?".to_string());
...
mod tests {
    ...
    fn test_duplicate_detection() {
        ...
        // Check for duplicate
        let duplicate = db.check_duplicate(
            "K1DEF",
            14074000.0,
    ...
    fn test_filter_queries() {
        ...
        let results = db.search_qsos(&filter, &QueryOptions::default()).unwrap();
        assert_eq!(results.len(), 1);
        assert_eq!(results[0].metadata.their_callsign, Some("VE3XYZ".to_string()));
    }
}