...
impl AdifProcessor {
    ...
    pub fn parse_string(&self, data: &str) -> Result<AdifFile, AdifError> {
        ...
        let header = AdifHeader {
            version: header_fields.get("ADIF_VER")
                .map(|f| f.value.clone())
                .unwrap_or_else(|| "3.1.0".to_string()),
            program_id: header_fields.get("PROGRAMID")
                .map(|f| f.value.clone())
                .unwrap_or_else(|| "pancetta-qso".to_string()),
            program_version: header_fields.get("PROGRAMVERSION")
                .map(|f| f.value.clone())
                .unwrap_or_else(|| "0.1.0".to_string()),
            created_timestamp: header_fields.get("CREATED_TIMESTAMP")
    ...
    pub fn generate_string(&self, file: &AdifFile) -> Result<String, AdifError> {
        ...
        output.push_str(&format!("Generated on {}\n\n", 
                                 file.header.created_timestamp.format("%Y-%m-%d %H:%M:%S UTC")));
        ...
        output.push_str(&self.format_field("CREATED_TIMESTAMP", 
                                          &file.header.created_timestamp.format("%Y%m%d %H%M%S").to_string())?);
    ...
    pub fn generate_writer<W: Write>(&self, file: &AdifFile, mut writer: W) -> Result<(), AdifError> {
    ...
    pub fn qso_to_adif(&self, metadata: &QsoMetadata, contest_info: Option<&ContestInfo>) -> AdifQso {
        ...
        AdifQso {
            ...
            mode: if metadata.mode == "FT8" { "DATA".to_string() } else { metadata.mode.clone() },
            submode: if metadata.mode == "FT8" { Some("FT8".to_string()) } else { None },
            freq: freq_mhz,
            band,
            rst_sent: metadata.reports.sent.map(|r| self.signal_report_to_rst(r)),
            rst_rcvd: metadata.reports.received.map(|r| self.signal_report_to_rst(r)),
    ...
    pub fn adif_to_qso(&self, adif_qso: &AdifQso) -> QsoMetadata {
        ...
        QsoMetadata {
            ...
            their_callsign: if adif_qso.call.is_empty() { None } else { Some(adif_qso.call.clone()) },
            frequency,
            mode: adif_qso.submode.clone().unwrap_or_else(|| adif_qso.mode.clone()),
            start_time: adif_qso.qso_date,
            end_time: adif_qso.qso_date_off,
            reports: SignalReports {
                sent: adif_qso.rst_sent.as_ref().and_then(|r| self.rst_to_signal_report(r)),
                received: adif_qso.rst_rcvd.as_ref().and_then(|r| self.rst_to_signal_report(r)),
            },
            grids: GridSquares {
                ours: adif_qso.my_gridsquare.clone(),
                theirs: adif_qso.gridsquare.clone(),
            },
            contest_info: adif_qso.contest_id.as_ref().map(|contest_name| ContestInfo {
    ...
    pub fn record_to_qso(&self, record: &AdifRecord) -> Result<AdifQso, AdifError> {
        ...
        Ok(AdifQso {
            ...
            mode: self.get_field_value(record, "MODE").unwrap_or_else(|| "DATA".to_string()),
            submode: self.get_field_value(record, "SUBMODE"),
            freq: self.get_field_value(record, "FREQ")
            ...
            tx_pwr: self.get_field_value(record, "TX_PWR").and_then(|p| p.parse().ok()),
            station_callsign: self.get_field_value(record, "STATION_CALLSIGN").unwrap_or_default(),
            ...
            dxcc: self.get_field_value(record, "DXCC").and_then(|d| d.parse().ok()),
            cqz: self.get_field_value(record, "CQZ").and_then(|z| z.parse().ok()),
            ituz: self.get_field_value(record, "ITUZ").and_then(|z| z.parse().ok()),
            state: self.get_field_value(record, "STATE"),
            contest_id: self.get_field_value(record, "CONTEST_ID"),
            stx: self.get_field_value(record, "STX").and_then(|s| s.parse().ok()),
            stx_string: self.get_field_value(record, "STX_STRING"),
            srx: self.get_field_value(record, "SRX").and_then(|s| s.parse().ok()),
    ...
    pub fn qso_to_record(&self, qso: &AdifQso) -> AdifRecord {
        ...
        self.add_field(&mut fields, "QSO_DATE", &qso.qso_date.format("%Y%m%d").to_string());
        self.add_field(&mut fields, "TIME_ON", &qso.qso_date.format("%H%M%S").to_string());
        ...
        if let Some(end_time) = qso.qso_date_off {
            self.add_field(&mut fields, "QSO_DATE_OFF", &end_time.format("%Y%m%d").to_string());
            self.add_field(&mut fields, "TIME_OFF", &end_time.format("%H%M%S").to_string());
        }
    ...
    fn parse_header_fields(&self, header: &str, line_num: usize) -> Result<HashMap<String, AdifField>, AdifError> {
    ...
    fn parse_fields(&self, data: &str, line_num: usize) -> Result<HashMap<String, AdifField>, AdifError> {
        let mut fields = HashMap::new();
    ...
    fn format_field(&self, name: &str, value: &str) -> Result<String, AdifError> {
        ...
        Ok(format!("<{}:{}>{}", name.to_uppercase(), value.len(), value))
    }
    ...
    fn parse_qso_datetime(&self, record: &AdifRecord) -> Result<DateTime<Utc>, AdifError> {
        ...
        let time = self.get_field_value(record, "TIME_ON").unwrap_or_else(|| "000000".to_string());
    ...
    fn parse_optional_datetime(
        ...
        if let Some(date) = self.get_field_value(record, date_field) {
            let time = self.get_field_value(record, time_field).unwrap_or_else(|| "000000".to_string());
            let datetime_str = format!("{} {}", date, time);
    ...
    fn get_required_field(&self, record: &AdifRecord, field_name: &str) -> Result<String, AdifError> {
        record.fields.get(&field_name.to_uppercase())
            .map(|f| f.value.clone())
    ...
    fn get_field_value(&self, record: &AdifRecord, field_name: &str) -> Option<String> {
        record.fields.get(&field_name.to_uppercase()).map(|f| f.value.clone())
    }
    ...
    fn add_field(&self, fields: &mut HashMap<String, AdifField>, name: &str, value: &str) {
        if !value.is_empty() {
            fields.insert(name.to_uppercase(), AdifField {
    ...
    fn add_optional_field(&self, fields: &mut HashMap<String, AdifField>, name: &str, value: &Option<String>) {
