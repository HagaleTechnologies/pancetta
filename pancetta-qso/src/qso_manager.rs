...
impl QsoManager {
    ...
    pub fn new(config: QsoManagerConfig) -> Self {
        ...
        let next_serial = config.contest_mode
            .as_ref()
    ...
    pub async fn respond_to_cq(
        ...
        info!("Responding to CQ from {} on {:.1} Hz: {}", target_callsign, frequency, qso_id);
    ...
    pub async fn cancel_qso(&self, qso_id: QsoId) -> Result<(), QsoManagerError> {
        ...
        if let Some(mut progress) = qsos.remove(&qso_id) {
            ...
            self.emit_state_change(qso_id, old_state, progress.state.clone()).await;
    ...
    async fn process_message_for_qso(
        ...
        let mut qsos = self.qsos.write().await;
        let progress = qsos.get_mut(&qso_id)
            .ok_or(QsoManagerError::QsoNotFound { qso_id })?;
        ...
        let new_state = self.determine_state_transition(&old_state, &message.message_type).await?;
        ...
        self.emit_event(QsoEvent::MessageReceived {
            qso_id,
            message,
        }).await;
    ...
    async fn determine_state_transition(
        ...
        match (current_state, message_type) {
            ...
            (QsoState::CallingCq { frequency, .. }, MessageType::CqResponse { responding_station, .. }) => {
            ...
            (QsoState::RespondingToCq { target_callsign, frequency, .. }, MessageType::SignalReport { report, .. }) => {
            ...
            (QsoState::SendingReport { their_callsign, their_report, our_report, frequency, .. }, 
            ...
            (QsoState::WaitingForConfirmation { their_callsign, their_report, our_report, frequency, grid_square, started_at }, 
    ...
    async fn find_qsos_for_message(&self, message_type: &MessageType, frequency: f64) -> Vec<QsoId> {
        let qsos = self.qsos.read().await;
    ...
    fn is_message_relevant(&self, state: &QsoState, message_type: &MessageType, frequency: f64) -> bool {
        // Check frequency match (with tolerance)
        if let Some(qso_freq) = state.frequency() {
            if (qso_freq - frequency).abs() > 50.0 { // 50 Hz tolerance
                return false;
            }
        }
        ...
        match (state, message_type) {
            (QsoState::CallingCq { .. }, MessageType::CqResponse { calling_station, .. }) => {
                calling_station == &self.config.our_callsign
            }
            
            (QsoState::RespondingToCq { target_callsign, .. }, MessageType::SignalReport { to_station, .. }) => {
    ...
    async fn check_duplicate(&self, callsign: &str, frequency: f64) -> Result<bool, QsoManagerError> {
        ...
        if let Some(qso_ids) = qsos_by_callsign.get(callsign) {
            let qsos = self.qsos.read().await;
            let time_window = Duration::hours(self.config.duplicate_checking.time_window_hours as i64);
    ...
    async fn add_callsign_mapping(&self, callsign: &str, qso_id: QsoId) {
        ...
        qsos_by_callsign.entry(callsign.to_string())
    ...
    async fn cleanup_completed_qsos(&self) {
        ...
        let to_remove: Vec<QsoId> = qsos.iter()
            .filter(|(_, progress)| {
        ...
        for (&qso_id, progress) in qsos.iter() {
            if let Some(duration) = progress.state.state_duration(now) {
                let timeout_seconds = match &progress.state {
                    ...
                    QsoState::WaitingForConfirmation { .. } => self.config.timeouts.confirmation_timeout,
                    _ => continue,
                };
...
mod tests {
    ...
    async fn test_respond_to_cq() {
        let manager = QsoManager::new(test_config());
        let qso_id = manager.respond_to_cq("K1DEF".to_string(), 14074000.0).await.unwrap();
