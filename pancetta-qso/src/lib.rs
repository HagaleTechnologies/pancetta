...
// Deterministic QSO-engine test/simulation harness. `pub` so integration tests
// in other crates can drive it, but `#[doc(hidden)]` — it is internal test
// infrastructure, not part of the supported public API, and must not appear in
// the published rustdoc surface.
#[doc(hidden)]
pub mod sim;
