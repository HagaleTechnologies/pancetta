...
use crate::states::*;
use crate::adif::{AdifProcessor, AdifFile};
...
use crate::logger::{LoggerConfig, ExportResult, ImportResult, ExportFormat};
...
impl AsyncQsoLogger {
    ...
    pub async fn export_adif<P: AsRef<Path>>(
        ...
        for progress in &records {
            let adif_qso = self.adif_processor.qso_to_adif(
                &progress.metadata,
    ...
    pub async fn import_adif<P: AsRef<Path>>(
        ...
        info!("Imported {} QSOs from ADIF ({} duplicates, {} errors) in {}ms",
    ...
    pub async fn get_statistics(&self) -> Result<DatabaseStatistics, AsyncLoggerError> {
        ...
        let oldest = self.database.search_qsos(&filter, &options).await?
            .first()
            .map(|q| q.metadata.start_time);
        
        let newest = self.database.search_qsos(&filter, &options).await?
            .first()
            .map(|q| q.metadata.start_time);
    ...
    fn adif_qso_to_progress(&self, adif_qso: &crate::adif::AdifQso) -> QsoProgress {
        QsoProgress {
            state: QsoState::Completed {
                ...
                their_report: adif_qso.rst_rcvd.as_ref()
                    .and_then(|r| r.parse().ok())
                    .unwrap_or(0),
                our_report: adif_qso.rst_sent.as_ref()
                    .and_then(|r| r.parse().ok())
                    .unwrap_or(0),
