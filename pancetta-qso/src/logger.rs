...
use crate::states::*;
use crate::adif::{AdifProcessor, AdifFile, AdifError};
use crate::database::{QsoFilter, QueryOptions, DatabaseError};
use crate::async_database::{AsyncQsoDatabase, AsyncDatabaseError};
use crate::qso_manager::{QsoManager, QsoEvent};
...
pub enum LoggerError {
    ...
    QsoManager { source: crate::qso_manager::QsoManagerError },
...
impl QsoLogger {
    ...
    pub async fn new(config: LoggerConfig, qso_manager: QsoManager) -> Result<Self, LoggerError> {
        info!("Initializing QSO logger with database: {:?}", config.database_path);
    ...
    pub async fn log_qso(&self, progress: &QsoProgress) -> Result<i64, LoggerError> {
        ...
        let id = self.database.insert_qso(progress)
            .await
            .map_err(|e| LoggerError::AsyncDatabase { source: e })?;
        
        info!("Logged QSO: {} -> {} (id: {})", 
              progress.metadata.our_callsign,
              progress.metadata.their_callsign.as_deref().unwrap_or("UNKNOWN"),
              id);
    ...
    pub async fn update_qso(&self, progress: &QsoProgress) -> Result<(), LoggerError> {
        ...
        self.database.update_qso(progress)
            .await
            .map_err(|e| LoggerError::AsyncDatabase { source: e })?;
    ...
    pub async fn export_adif<P: AsRef<Path>>(
        ...
        let records = self.database.search_qsos_records(filter, &options)
            .await
            .map_err(|e| LoggerError::AsyncDatabase { source: e })?;
        ...
        let adif_file = AdifFile {
            ...
            records: adif_qsos.iter()
                .map(|qso| self.adif_processor.qso_to_record(qso))
                .collect(),
        };
        
        // Write to file
        let adif_data = self.adif_processor.generate_string(&adif_file)
            .map_err(|e| LoggerError::Adif { source: e })?;
        
        fs::write(&output_path, &adif_data)
            .map_err(|e| LoggerError::Io { source: e })?;
        ...
        self.export_history.write().await.push(export_result.clone());
        
        info!("Exported {} QSOs to ADIF in {}ms", records.len(), duration_ms);
    ...
    pub async fn import_adif<P: AsRef<Path>>(
        ...
        let adif_data = fs::read_to_string(&input_path)
            .map_err(|e| LoggerError::Io { source: e })?;
        
        let adif_file = self.adif_processor.parse_string(&adif_data)
            .map_err(|e| LoggerError::Adif { source: e })?;
        ...
        for (index, record) in adif_file.records.iter().enumerate() {
            match self.import_adif_record(record).await {
                ...
                Err(e) => {
                    ...
                    if errors.len() > 100 { // Limit error list size
        ...
        self.import_history.write().await.push(import_result.clone());
        
        info!("Imported {} QSOs, skipped {}, errors {} in {}ms", 
    ...
    pub async fn export_csv<P: AsRef<Path>>(
        ...
        // Header
        csv_content.push_str("QSO_DATE,TIME_ON,CALL,MODE,FREQ,BAND,RST_SENT,RST_RCVD,GRIDSQUARE,COMMENT\n");
        ...
        info!("Exported {} QSOs to CSV in {}ms", records.len(), duration_ms);
    ...
    pub async fn search_qsos(
        ...
        self.database.search_qsos_records(filter, options)
            .await
            .map_err(|e| LoggerError::AsyncDatabase { source: e })
    }
    ...
    async fn auto_logging_loop(&self) {
        let mut interval_timer = interval(Duration::from_millis(self.config.auto_logging.log_interval));
    ...
    async fn handle_qso_completed(&self, qso_id: QsoId, metadata: QsoMetadata) -> Result<(), LoggerError> {
    ...
    async fn should_auto_log(&self, progress: &QsoProgress) -> bool {
        ...
        if let Some(duration) = progress.metadata.end_time.zip(Some(progress.metadata.start_time))
        ...
        if config.require_both_reports {
            if progress.metadata.reports.sent.is_none() || progress.metadata.reports.received.is_none() {
                return false;
            }
        }
    ...
    async fn validate_qso(&self, progress: &QsoProgress) -> Result<(), LoggerError> {
        ...
        if config.check_duplicates {
            if let Some(ref callsign) = progress.metadata.their_callsign {
                if let Some(duplicate_id) = self.database.check_duplicate(
                    callsign,
                    progress.metadata.frequency,
                    progress.metadata.start_time,
                    config.duplicate_window_hours,
                ).await.map_err(|e| LoggerError::AsyncDatabase { source: e })? {
                    return Err(LoggerError::Validation {
    ...
    fn is_valid_callsign(&self, callsign: &str) -> bool {
        ...
        callsign.len() >= 3 && callsign.len() <= 10 && 
        callsign.chars().all(|c| c.is_ascii_alphanumeric() || c == '/')
    }
    
    async fn auto_export_latest(&self) -> Result<(), LoggerError> {
        if self.config.export_import.export_formats.contains(&ExportFormat::Adif) {
            let timestamp = Utc::now().format("%Y%m%d_%H%M%S");
    ...
    async fn import_adif_record(
        ...
        let adif_qso = self.adif_processor.record_to_qso(record)
            .map_err(|e| LoggerError::Adif { source: e })?;
        ...
        if let Some(ref callsign) = metadata.their_callsign {
            if let Some(_) = self.database.check_duplicate(
                callsign,
                metadata.frequency,
                metadata.start_time,
        ...
        let progress = QsoProgress {
            state: QsoState::Completed {
                ...
                duration_seconds: metadata.end_time
        ...
        // Insert QSO
        self.database.insert_qso(&progress)
            .await
            .map_err(|e| LoggerError::AsyncDatabase { source: e })?;
    ...
    async fn cleanup_old_backups(&self) -> Result<(), LoggerError> {
        ...
        if let Ok(entries) = fs::read_dir(backup_dir) {
            for entry in entries.flatten() {
                if let Ok(metadata) = entry.metadata() {
                    if metadata.is_file() && 
                       entry.file_name().to_string_lossy().starts_with("qso_backup_") {
        ...
        if backups.len() > self.config.backup.retain_count as usize {
            for (path, _) in backups.into_iter().skip(self.config.backup.retain_count as usize) {
...
mod tests {
    use super::*;
    use crate::qso_manager::{QsoManagerConfig, TimeoutConfig, AutoSequenceConfig, DuplicateCheckConfig};
    ...
    async fn create_test_logger() -> QsoLogger {
        ...
        QsoLogger::new(test_logger_config(), qso_manager).await.unwrap()
    }
