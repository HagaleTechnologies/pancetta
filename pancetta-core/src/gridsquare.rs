...
use crate::types::{PancettaError, PancettaResult};

/// Convenience alias for grid-related results.
type Result<T> = PancettaResult<T>;
...
impl GridSquare {
    ...
    pub fn new(grid: &str) -> Result<Self> {
        ...
        let precision = match grid.len() {
            ...
            _ => {
                return Err(PancettaError::InvalidInput(format!(
...
pub fn grid_to_coordinates(grid: &str) -> Result<(f64, f64)> {
    ...
    let precision = match chars.len() {
        ...
        8 => GridPrecision::ExtendedSquare,
        10 => GridPrecision::SubExtendedSquare,
        _ => return Err(PancettaError::InvalidInput("Invalid length".to_string())),
    };
