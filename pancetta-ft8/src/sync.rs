...
use crate::{
    ...
    signal_processing::{FftProcessor, WindowFunction, SymbolCorrelator},
};
...
impl TimeSync {
    ...
    pub fn synchronize(&mut self, audio_data: &[f32], capture_time: SystemTime) -> Ft8Result<SyncResult> {
    ...
    fn validate_utc_timing(&self, sync_result: SyncResult, capture_time: SystemTime) -> Ft8Result<SyncResult> {
        ...
        let timing_error = actual_sync.duration_since(expected_sync)
        ...
        if timing_error <= SYNC_TOLERANCE {
            ...
            validated.confidence *= 1.0 - (timing_error / SYNC_TOLERANCE) * 0.2; // Reduce confidence based on error
        } else {
    ...
    fn calculate_sync_confidence(&self, correlation: f64, symbol_alignment: f64) -> f64 {
        ...
        let correlation_confidence = (correlation - MIN_SYNC_CORRELATION) / (1.0 - MIN_SYNC_CORRELATION);
    ...
    fn calculate_quality_metrics(&self) -> Ft8Result<SyncQualityMetrics> {
        ...
        metrics.peak_correlation = self.sync_history.iter()
            .map(|m| m.correlation)
            .fold(0.0f64, f64::max);
        
        // Calculate stability (inverse of timing variance)
        let mean_offset: f64 = self.sync_history.iter().map(|m| m.offset).sum::<f64>() / self.sync_history.len() as f64;
        let variance: f64 = self.sync_history.iter()
            .map(|m| (m.offset - mean_offset).powi(2))
            .sum::<f64>() / self.sync_history.len() as f64;
        
        metrics.timing_jitter = variance.sqrt();
        metrics.stability = if variance > 0.0 { 1.0 / (1.0 + variance) } else { 1.0 };
    ...
    pub fn time_to_next_sync(&self) -> Duration {
        self.next_sync_time.duration_since(SystemTime::now())
...
fn calculate_expected_sync_time(capture_time: SystemTime) -> SystemTime {
    let since_epoch = capture_time.duration_since(UNIX_EPOCH)
        .unwrap_or(Duration::ZERO);
