    ...
    pub fn encode(&self, message_bits: &BitSlice) -> Ft8Result<BitVec> {
        if message_bits.len() != LDPC_INFO_BITS {
            return Err(Ft8Error::MessageDecodingError(
                format!("Invalid LDPC input length: {} (expected {})", message_bits.len(), LDPC_INFO_BITS)
            ));
        }
        ...
        for j in 0..LDPC_CODEWORD_BYTES {
            codeword_bytes[j] = if j < LDPC_INFO_BYTES { message_bytes[j] } else { 0 };
        }
...
mod tests {
    ...
    fn test_parity_check_matrix_creation() {
        ...
        for check_idx in 0..LDPC_NUM_CHECKS {
            ...
            assert_eq!(
                connections.len(), expected,
                "Check node {} has {} connections, expected {}",
                check_idx, connections.len(), expected
            );
        }
    ...
    fn test_ldpc_encode_all_zeros() {
        ...
        assert!(codeword.not_any(), "All-zero input should produce all-zero codeword");
        assert!(encoder.verify_syndrome(&codeword));
    }
    ...
    fn test_ldpc_encode_deterministic() {
        ...
        assert_eq!(codeword1, codeword2, "Same input should produce same output");
    }
    ...
    fn test_gray_code_round_trip() {
        for b in 0..8u8 {
            ...
            assert_eq!(b, back, "Gray round-trip failed for {}: gray={}, back={}", b, gray, back);
        }
    }
    ...
    fn test_gray_code_is_bijection() {
        ...
        for i in 0..8u8 {
            ...
            assert!(!seen[g as usize], "binary_to_gray({}) = {} is duplicate", i, g);
    ...
    fn test_gray_code_known_values() {
        ...
        for (b, &expected) in expected_gray.iter().enumerate() {
            assert_eq!(
                binary_to_gray(b as u8), expected,
                "binary_to_gray({}) = {} but expected {}",
                b, binary_to_gray(b as u8), expected
            );
        }
    }
        ...
        for check_idx in 0..LDPC_NUM_CHECKS {
            ...
            for col in 0..num_active {
                ...
                assert!(
                    ...
                    check_idx, var_0origin, var_0origin, LDPC_MN[var_0origin], check_1origin
                );
            }
        }
    }
