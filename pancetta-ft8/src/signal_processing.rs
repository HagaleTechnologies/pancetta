...
use crate::{Ft8Error, Ft8Result, SAMPLE_RATE, SYMBOL_DURATION, NUM_TONES, TONE_SPACING, BASE_FREQUENCY};
...
impl FftProcessor {
    ...
    pub fn fft_real(&mut self, input: &[f64]) -> Ft8Result<&[Complex<f64>]> {
        if input.len() > self.fft_size {
            return Err(Ft8Error::FftError(
                format!("Input size {} exceeds FFT size {}", input.len(), self.fft_size)
            ));
        }
    ...
    pub fn fft_complex(&mut self, input: &[Complex<f64>]) -> Ft8Result<&[Complex<f64>]> {
        if input.len() != self.fft_size {
            return Err(Ft8Error::FftError(
                format!("Input size {} doesn't match FFT size {}", input.len(), self.fft_size)
            ));
        }
...
fn design_bandpass_filter(center_freq: f64, bandwidth: f64, order: usize) -> Ft8Result<Vec<f64>> {
    ...
    for i in 0..order {
        ...
        if n == 0 {
            ...
            let n_f = n as f64;
            coeffs[i] = (2.0 * PI * f2 * n_f).sin() / (PI * n_f) - 
                       (2.0 * PI * f1 * n_f).sin() / (PI * n_f);
        }
