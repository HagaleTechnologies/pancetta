...
use pancetta_ft8::{
    Ft8Decoder, Ft8Config,
    SAMPLE_RATE, WINDOW_SAMPLES, SYMBOL_DURATION, NUM_SYMBOLS,
};
use std::f64::consts::PI;

/// Generate synthetic FT8 signal for benchmarking
fn generate_benchmark_signal(
    num_signals: usize,
    base_snr: f32,
...
fn benchmark_configuration_variants(c: &mut Criterion) {
    ...
    group.bench_function("aggressive", |b| {
        b.iter(|| {
            black_box(aggressive_decoder.decode_window(black_box(&samples)).unwrap());
        });
    });
...
fn benchmark_signal_processing(c: &mut Criterion) {
    use pancetta_ft8::signal_processing::{FftProcessor, WindowFunction, BandpassFilter};
    ...
    group.bench_function("psd_4096", |b| {
        b.iter(|| {
            black_box(fft_processor.power_spectral_density(black_box(&test_data)).unwrap());
        });
    });
    ...
    group.bench_function("bandpass_filter", |b| {
        b.iter(|| {
            black_box(filter.filter_batch(black_box(&filter_test_data), black_box(&mut output)).unwrap());
        });
    });
...
fn benchmark_time_sync(c: &mut Criterion) {
    ...
    let audio_data: Vec<f64> = (0..WINDOW_SAMPLES).map(|i| (i as f64 / 1000.0).sin()).collect();
    let audio_f32: Vec<f32> = audio_data.iter().map(|&x| x as f32).collect();
    
    group.bench_function("synchronization", |b| {
        b.iter(|| {
            black_box(time_sync.synchronize(
                black_box(&audio_f32),
...
fn benchmark_realtime_scenarios(c: &mut Criterion) {
    ...
    group.bench_function("contest_conditions", |b| {
        b.iter(|| {
            black_box(decoder.decode_window(black_box(&contest_conditions)).unwrap());
        });
    });
...
fn benchmark_throughput(c: &mut Criterion) {
    ...
    for i in 0..num_windows {
        ...
        windows.push(generate_benchmark_signal(signals_per_window, base_snr, 60.0));
    }
