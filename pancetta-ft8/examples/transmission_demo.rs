...
use pancetta_ft8::{
    Ft8Encoder, Ft8Modulator, Ft8Transmitter, TransmissionConfig,
    AudioFormat, convert_samples, ModulatorConfig,
    NUM_SYMBOLS, MESSAGE_DURATION, SAMPLE_RATE
};
...
fn demo_message_encoding() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (message, description) in messages {
        match encoder.encode_message(message, None) {
            Ok(symbols) => {
                println!("✅ {} '{}': {} symbols", description, message, symbols.len());
                print!("   Symbols: ");
...
fn demo_audio_modulation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for (name, format) in formats {
        ...
        println!("  {}: {} bytes ({} bytes/sample)", 
                 name, converted.len(), format.bytes_per_sample());
    }
...
fn demo_transmission_system() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("  Max TX time: {} seconds", config.safety_config.max_tx_time_seconds);
    println!("  Band edges: {:.0} - {:.0} Hz", 
    ...
    println!("  Remaining TX time: {} seconds", stats.remaining_tx_time_seconds);
...
async fn async_demo() -> Result<(), Box<dyn std::error::Error>> {
    ...
    match transmitter.test_transmission_system(0.1).await {
        Ok(test_report) => {
            ...
            println!("  PTT test: {}", if test_report.ptt_test.success { "PASS" } else { "FAIL" });
            println!("  Audio test: {}", if test_report.audio_test.success { "PASS" } else { "FAIL" });
            println!("  Frequency test: {}", if test_report.frequency_test.within_tolerance { "PASS" } else { "FAIL" });
