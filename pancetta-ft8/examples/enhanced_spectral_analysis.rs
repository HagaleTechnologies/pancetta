...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Processing {} samples ({:.2} seconds)",
             audio.len(), audio.len() as f64 / SAMPLE_RATE as f64);
    ...
    let waterfall = decoder.generate_waterfall_data(&audio.iter().map(|&x| x as f64).collect::<Vec<_>>())?;
    
    println!("Waterfall data:");
    println!("  Time bins: {}", waterfall.time_bins.len());
    println!("  Frequency bins: {}", waterfall.frequency_bins.len());
    println!("  Power range: {:.1} to {:.1} dB", waterfall.min_power, waterfall.max_power);
    ...
    if !waterfall.power_matrix.is_empty() {
        ...
        for (i, freq) in waterfall.frequency_bins.iter().enumerate() {
            ...
            if count > 0 {
                ...
                if avg_power > waterfall.min_power + 10.0 { // 10 dB above noise floor
