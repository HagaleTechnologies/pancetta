...
pub struct VerifiedArmGrant {
    ...
    /// authorization refresh (PAN-18) can tell whether the CURRENT arm's
    /// controller is still TX-arm-eligible, independent of the ham-radio
    /// `operator_callsign` attribution above.
...
pub enum DisarmReason {
    ...
    ComponentCrash,
    /// A live cqdx authorization refresh (PAN-18) removed the current arm's
    /// controller from TX-arm eligibility — either the edge was revoked
    /// entirely or it became delegated. The cloud may only ever REDUCE
    /// station-local TX authority (frozen e2e-auth.v1 §6-revocation), so a
    /// poll that demotes an already-armed controller must retroactively
    /// disarm rather than leave the live grant to run out its TTL.
...
impl ArmState {
    ...
    /// The client keyId of the currently-armed grant, if armed (PAN-18). This
    /// is the cryptographic client identity the grant was verified for —
    /// distinct from [`Self::operator_callsign`], which is the ham-radio
    /// callsign attribution. Used by the Q-0043 authorization-poll task to
    /// detect that a live cqdx refresh has removed the current controller
    /// from TX-arm eligibility (revoked or newly delegated), so the arm can
    /// be disarmed immediately rather than left to run out its TTL.
...
mod tests {
    ...
    /// PAN-18: `armed_client_key_id()` exposes the CRYPTOGRAPHIC client
    /// identity the live arm was verified for (distinct from
    /// `operator_callsign()`, the ham-radio attribution) — `None` when not
    /// armed, `Some(client_key_id)` while armed, and back to `None` once
