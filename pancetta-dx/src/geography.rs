...
impl Coordinate {
    ...
    pub fn new(latitude: f64, longitude: f64) -> Result<Self> {
        if !(-90.0..=90.0).contains(&latitude) {
            return Err(DxError::Geography(format!(
                "Invalid latitude: {}",
                latitude
            )));
        }
...
impl GeographyCalculator {
    ...
    pub fn new(home_latitude: f64, home_longitude: f64) -> Self {
        let home_coordinate =
            Coordinate::new(home_latitude, home_longitude).unwrap_or_else(|_| Coordinate {
                latitude: 0.0,
                longitude: 0.0,
            });
    ...
    pub fn calculate_distance_between(
        ...
        let (distance_m, forward_azimuth, reverse_azimuth, _arc) =
            self.geodesic.inverse(lat1, lon1, lat2, lon2);
    ...
    pub fn calculate_bearing_between(
        ...
        let (_distance, forward_azimuth, _reverse, _arc) =
            self.geodesic.inverse(lat1, lon1, lat2, lon2);
    ...
    pub fn calculate_midpoint(&self, lat1: f64, lon1: f64, lat2: f64, lon2: f64) -> Coordinate {
        ...
        let mid_lat = (lat1_rad.sin() + lat2_rad.sin())
    ...
    pub fn calculate_destination(
        ...
        let (end_lat, end_lon, _end_azimuth) =
            self.geodesic
                .direct(start_lat, start_lon, bearing_degrees, distance_m);
    ...
    pub fn calculate_cross_track_distance(
        ...
        let dist_to_start = self
            .calculate_distance_between(point_lat, point_lon, path_start_lat, path_start_lon)
            .distance_km;

        // Calculate bearing from start to point
        let bearing_to_point = self
            .calculate_bearing_between(path_start_lat, path_start_lon, point_lat, point_lon)
            .true_bearing;

        // Calculate bearing of the path
        let path_bearing = self
            .calculate_bearing_between(path_start_lat, path_start_lon, path_end_lat, path_end_lon)
            .true_bearing;
    ...
    pub fn is_within_radius(
        ...
        let distance = self
            .calculate_distance_between(point_lat, point_lon, center_lat, center_lon)
            .distance_km;
    ...
    pub fn closest_point_on_path(
        ...
        let dist_to_point =
            self.calculate_distance_between(path_start_lat, path_start_lon, point_lat, point_lon);
        ...
        let path_distance = self
            .calculate_distance_between(path_start_lat, path_start_lon, path_end_lat, path_end_lon)
            .distance_km;
...
pub fn calculate_sun_times(
    ...
    let sunrise = if (0.0..24.0).contains(&sunrise_hour) {
        ...
        let adjusted_date = if sunrise_hour < 0.0 {
    ...
    let sunset = if (0.0..24.0).contains(&sunset_hour) {
        ...
        let minute = ((adjusted_hour % 1.0) * 60.0) as u32;
        let second = (((adjusted_hour % 1.0) * 60.0 % 1.0) * 60.0) as u32;
        let adjusted_date = if sunset_hour < 0.0 {
            date - chrono::Duration::days(1)
        } else {
            date + chrono::Duration::days(1)
        };
...
fn mgrs_from_coordinate(lat: f64, lon: f64) -> Result<String> {
    ...
    Ok(format!(
        "{:02}{}{}{}{:04}{:04}",
        zone, zone_letter, grid_e, grid_n, e_coord, n_coord
    ))
}
