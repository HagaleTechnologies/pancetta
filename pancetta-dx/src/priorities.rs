...
use crate::{Band, Mode, DxSpot, DxPriorityConfig, DxError, Result};
...
impl PriorityManager {
    ...
    pub fn add_rule(&mut self, mut rule: PriorityRule) -> Result<()> {
        ...
        if let Some(pattern) = &rule.callsign_pattern {
            let regex = Regex::new(pattern)
                .map_err(|e| DxError::Configuration(format!("Invalid regex pattern '{}': {}", pattern, e)))?;
        ...
        if let Some(rarity_score) = spot.rarity_score {
            if rarity_score < self.config.min_rarity_score {
                return Ok(PriorityResult {
                    ...
                    contributing_goals: Vec::new(),
                    reason: format!("Rarity score {} below minimum {}", rarity_score, self.config.min_rarity_score),
                    alert: false,
                });
            }
                ...
                match rule.action {
                    ...
                    RuleAction::SetPriority => {
                        if rule.priority_level > final_priority {
                            ...
                            reason_parts.push(format!("Priority set by rule '{}': {}", rule.name, rule.priority_level));
                        }
                    },
                }
            }
    ...
    fn sort_rules(&mut self) {
        self.rules.sort_by(|a, b| b.rule_priority.cmp(&a.rule_priority));
    }
    ...
    pub fn get_priority_stats(&self) -> HashMap<String, usize> {
        ...
        stats.insert("enabled_rules".to_string(), self.rules.iter().filter(|r| r.enabled).count());
        stats.insert("total_goals".to_string(), self.goals.len());
        stats.insert("enabled_goals".to_string(), self.goals.iter().filter(|g| g.enabled).count());
...
mod tests {
    ...
    fn test_score_to_priority_conversion() {
        ...
        assert_eq!(manager.score_to_priority_level(0.95), PriorityLevel::Critical);
        assert_eq!(manager.score_to_priority_level(0.85), PriorityLevel::VeryHigh);
        ...
        assert_eq!(manager.score_to_priority_level(0.15), PriorityLevel::VeryLow);
