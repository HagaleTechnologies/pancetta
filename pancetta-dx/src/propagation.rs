...
impl PropagationPredictor {
    ...
    async fn fetch_noaa_indices(&self) -> Result<SolarIndices> {
        ...
        let sfi = 150.0 + 50.0 * (current_time.ordinal() as f64 / 365.0 * 2.0 * std::f64::consts::PI).sin();
    ...
    pub async fn predict_propagation(
        ...
        let distance_km = self.calculate_great_circle_distance(
            source_lat, source_lon, dest_lat, dest_lon
        );
        
        debug!("Calculating propagation prediction for {:.0} km path", distance_km);
        ...
        let band_conditions = self.predict_band_conditions(&solar_indices, distance_km, prediction_time);
        
        // Calculate SNR predictions
        let snr_predictions = self.calculate_snr_predictions(&solar_indices, distance_km, &band_conditions);
    ...
    fn calculate_snr_predictions(
        ...
        for (&band, &condition) in conditions {
            ...
            let path_loss = 20.0 * (distance_km / 100.0).log10() + 20.0 * freq_center_mhz.log10() - 27.55;
...
mod tests {
    ...
    async fn test_propagation_prediction() {
        ...
        let prediction = predictor.predict_propagation(
    ...
    fn test_great_circle_distance() {
        ...
        let distance = predictor.calculate_great_circle_distance(
            40.7128, -74.0060,
            51.5074, -0.1278,
        );
